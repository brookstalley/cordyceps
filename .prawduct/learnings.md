...
## Build-plan `## Status` checkboxes are hand-ticked — nothing derives them

**This reverses the earlier rule in this file, and the reversal is the point.** Boxes were once a
derived view regenerated by `prawduct-hook regen-views` from change-log `status=shipped` tags, so
hand-flipping one was drift the Critic flagged. That mechanism is **retired**: `regen-views` is
inert (warns, writes nothing), and `status=` in the change log is a dead key. Ticking is now
yours — tick each box after its chunk's review passes. The **last** tick disarms the Stop hook's
Critic and reflection gates, so on a `cumulative-final` plan hold it until that review passes.

The durable lesson under both rules: **a checkbox is only as true as whoever last touched it.**
When the owning mechanism changed, every plan whose prose cited the old one became a plan nobody
was ticking — `build-plan-reliability.md` still sits six-unticked for work merged in PR #26.
