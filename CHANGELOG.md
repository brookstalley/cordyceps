...
- Consolidated ~25+ individual tools into 12 action-based tools for improved LLM discoverability
- Each tool now supports `action='help'` to list available actions and parameters
...
- Create custom PBR materials with color, metallic, roughness parameters
- Instantiate built-in material types (Metal, Glass, Plastic, Paint, Gem, Plaster, Emission)
...
- Each tool's `action='help'` lists all available operations

This pattern also matches how other MCP servers organize their tools, making Cordyceps more intuitive for LLMs already familiar with the MCP ecosystem.
